...
class Bags(Dataset):
    def __init__(self, frame, config, paired):
        ...
        self.config = config
        self.paired = paired
        self.gain_min = float(config.get('gain_min', .7))
        self.gain_max = float(config.get('gain_max', 1.3))
        if not (np.isfinite(self.gain_min) and np.isfinite(self.gain_max)
                and 0 < self.gain_min <= self.gain_max):
            raise ValueError('finite positive gain_min <= gain_max required')
    ...
    def __getitem__(self, index):
        ...
        if self.paired:
            audio = audio * np.float32(np.random.uniform(self.gain_min, self.gain_max))
    ...
    for windows, mask, _, index in batches:
        with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=device.type == 'cuda'):
            output = forward_bags(model, windows.to(device), mask.to(device),
                                  ...
                                  peak_normalize=config.get('peak_normalize', False))
...
def main():
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        for step, (windows, mask, targets, _) in enumerate(train_loader):
            ...
            with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=device.type == 'cuda'):
                logits = forward_bags(model, windows.to(device), mask.to(device), config['temperature'],
                                      config['window_chunk_size'], peak_normalize=config.get('peak_normalize', False))[:, 2]
