#!/usr/bin/env python3
"""Finalize v65: matched training ablation plus primary fixed-WPT-slot gate."""
import argparse
import json
...
def main():
    ...
    config_path = ROOT / 'configs/paired_gain_wpt_v65.yaml'
    config = yaml.safe_load(config_path.read_text())
    runs = ROOT / 'reports/paired_gain_wpt_v65'
    if args.wait_for_training:
        wait_for_training(runs, variants=('paired_gain',))
    candidate_dir = runs / 'paired_gain'
    ...
    if checkpoint['config'].get('peak_normalize', False):
        raise ValueError('v65 must not use peak normalization')
    if (checkpoint['config'].get('gain_min'), checkpoint['config'].get('gain_max')) != (.1, 1.3):
        raise ValueError('candidate checkpoint does not use the frozen gain range')
    ...
    report = dict(stage='development_only', primary_decision=slot_gate,
        ...
        normalization_enabled=False, training_gain_range=[.1, 1.3], other_four_slot_outputs_text_exact=True,
