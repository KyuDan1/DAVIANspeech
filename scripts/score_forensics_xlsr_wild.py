#!/usr/bin/env python3
"""Offline deterministic scoring for the Forensics XLS-R Wild detector."""

from __future__ import annotations

import argparse
import importlib.util
import math
import sys
from pathlib import Path

import numpy as np
import pandas as pd
import torch
from safetensors.torch import load_file
from tqdm import tqdm

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))

from pipeline import find_audio_files, load_audio  # noqa: E402


def load_model(model_dir: Path, device: torch.device) -> torch.nn.Module:
    spec = importlib.util.spec_from_file_location(
        "vendored_forensics_xlsr_wild", model_dir / "model.py"
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot load Forensics model from {model_dir}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    model = module.DeepfakeDetector()
    state = load_file(model_dir / "checkpoint_epoch_4.safetensors")
    missing, unexpected = model.load_state_dict(state, strict=False)
    # The published file carries a legacy two-layer ``projection`` module
    # which the published inference model intentionally does not instantiate.
    # Accept only those four documented surplus tensors; reject any other
    # structural mismatch.
    allowed_unexpected = {
        "projection.0.bias", "projection.0.weight",
        "projection.2.bias", "projection.2.weight",
    }
    if missing or set(unexpected) != allowed_unexpected:
        raise RuntimeError(
            f"Checkpoint mismatch: missing={missing[:5]}, "
            f"unexpected={unexpected[:5]}"
        )
    return model.eval().to(device)


def fixed_windows(audio: np.ndarray, length: int, count: int) -> np.ndarray:
    audio = np.asarray(audio, dtype=np.float32)
    if not len(audio):
        audio = np.zeros(1, dtype=np.float32)
    peak = float(np.max(np.abs(audio)))
    if peak > 0:
        audio = audio / (peak + 1e-8)
    if len(audio) < length:
        audio = np.tile(audio, math.ceil(length / len(audio)))[:length]
        return np.repeat(audio[None], count, axis=0)
    if count == 1:
        starts = np.asarray([(len(audio) - length) // 2])
    else:
        starts = np.rint(np.linspace(0, len(audio) - length, count)).astype(int)
    return np.stack([audio[start:start + length] for start in starts])


def logmeanexp(values: np.ndarray, temperature: float = 5.0) -> float:
    scaled = temperature * np.asarray(values, dtype=np.float64)
    maximum = float(scaled.max())
    return float(
        (maximum + np.log(np.exp(scaled - maximum).mean())) / temperature
    )


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--data-dir", type=Path, required=True)
    parser.add_argument("--audio-subdir", default="audio")
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument(
        "--model-dir", type=Path,
        default=ROOT / "models/external/forensics_xlsr_wild",
    )
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--windows", type=int, default=3)
    parser.add_argument("--window-samples", type=int, default=80_000)
    parser.add_argument("--file-batch-size", type=int, default=8)
    args = parser.parse_args()
    if args.windows < 1 or args.file_batch_size < 1:
        parser.error("--windows and --file-batch-size must be positive")
    if args.output.exists():
        raise FileExistsError(f"Refusing to overwrite {args.output}")

    device = torch.device(args.device)
    model = load_model(args.model_dir, device)
    paths = find_audio_files(args.data_dir / args.audio_subdir)
    records: list[dict[str, float | int | str]] = []
    for offset in tqdm(
        range(0, len(paths), args.file_batch_size), desc="Forensics-XLSR-Wild"
    ):
        batch_paths = paths[offset:offset + args.file_batch_size]
        windows = np.concatenate([
            fixed_windows(load_audio(path), args.window_samples, args.windows)
            for path in batch_paths
        ])
        values = torch.from_numpy(windows).to(device)
        with torch.inference_mode(), torch.autocast(
            device_type=device.type, dtype=torch.bfloat16,
            enabled=device.type == "cuda",
        ):
            # The released logit is P(real); negate it for a fake-oriented
            # ranking score consistent with all competition outputs.
            fake_logits = -model(values).float().reshape(
                len(batch_paths), args.windows
            )
        fake_logits = fake_logits.cpu().numpy()
        for path, scores in zip(batch_paths, fake_logits):
            records.append({
                "ID": path.stem,
                "FAKE_LOGIT_MEAN": float(scores.mean()),
                "FAKE_LOGIT_MAX": float(scores.max()),
                "FAKE_LOGIT_LME5": logmeanexp(scores),
                "WINDOWS": len(scores),
            })
    args.output.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(records).to_csv(args.output, index=False)
    print(f"Saved {len(records)} scores to {args.output}")


if __name__ == "__main__":
    main()
