...
if [[ $# -lt 2 || $# -gt 5 ]]; then
  echo "usage: $0 DATASET OUTPUT_ROOT [NUM_SHARDS] [GPU_COUNT] [GPU_OFFSET]" >&2
  exit 2
fi
...
training_args=()
case "$dataset" in
  forensic_call_train_v1|temporal_mixed_train_v1|temporal_mixed_train_v2|\
  external_mixed_train_v1|mixed_devvoice_train_v1|mixed_fmc_music_train_v1|\
  mixfake_music_train_v1|telephone_mixed_train_v1|\
...
for ((shard=0; shard<num_shards; shard++)); do
  ...
  CUDA_VISIBLE_DEVICES=$gpu "$python_bin" \
    ...
    --test-dir "$audio_dir" --output "$destination/shard_$shard.npz" \
    --num-shards "$num_shards" --shard-index "$shard" \
    "${training_args[@]}" >"$destination/shard_$shard.log" 2>&1 &
  pids+=("$!")
done
