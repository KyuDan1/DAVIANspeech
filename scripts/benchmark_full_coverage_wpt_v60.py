...
from full_coverage_wpt_inference import load_full_coverage_model, predict_one_audio
...
def main():
    ...
    model, config = load_full_coverage_model(args.model_dir, args.checkpoint, device, allow_smoke=args.allow_smoke)
    ...
    for seconds in [4, 30, 60]:
        ...
        started = time.perf_counter()
        for _ in range(3):
            score, windows = predict_one_audio(model, audios[seconds], config, device)
        torch.cuda.synchronize()
        elapsed = (time.perf_counter() - started) / 3
        first[seconds] = score
        rows.append(dict(duration=seconds, windows=windows, seconds_per_file=elapsed,
                         peak_allocated_gib=torch.cuda.max_memory_allocated() / 2**30,
                         peak_reserved_gib=torch.cuda.max_memory_reserved() / 2**30))
    reverse = {seconds: predict_one_audio(model, audios[seconds], config, device)[0] for seconds in [60, 30, 4]}
    if first != reverse:
    ...
    report = dict(gpu=torch.cuda.get_device_name(), checkpoint=str(args.checkpoint),
                  smoke=args.allow_smoke, load_seconds=load_seconds, files=rows,
