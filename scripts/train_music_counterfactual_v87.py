        ...
        with (args.output / f'epoch_{epoch}_traces.jsonl').open('w') as stream:
            for index in range(4 if args.smoke else CONFIG['draws_per_epoch']):
                ...
                for name, model in models.items():
                    ...
                    adapter_grads = [p.grad for p in model.encoder.adapters.parameters()]
                    assert all(g is not None and torch.isfinite(g).all() for g in adapter_grads)
                    # A correctly saturated BCE example can have exactly zero
                    # float32 gradient. Preserve graph/finite checks per step,
                    # and prove actual adapter updates across the full run below.
                    if not any(g.abs().sum() > 0 for g in adapter_grads):
                        print(json.dumps(dict(stage='zero_gradient_diagnostic', epoch=epoch,
                            draw=index, model=name, loss=float(loss.detach()),
                            logits=output.detach().cpu().tolist(), label=label, kind=trace['kind'])), flush=True)
                    opts[name].step()
