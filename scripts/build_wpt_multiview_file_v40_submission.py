#!/usr/bin/env python3
"""Build v40 by widening only the WPT File expert's temporal coverage."""
...
CALL_MARKER = '''        unified_expert, device=args.device, file_batch_size=8,
...
def main() -> None:
    ...
    parser.add_argument(
        "--base-zip", type=Path, default=ROOT / "wpt_fixed_moe_v39.zip"
    )
    ...
    if script.count(CALL_MARKER) != 1:
        raise ValueError("v39 WPT call marker changed; refusing unsafe injection")
