...
v18은 DACON API 제출에 성공했으며 실제 채점 결과를 기다리는 중이다. 남은 제출권을
보존하기 위해 v19는 결과 확인 전까지 제출하지 않는다.
...
v19는 로컬 기준으로 v18보다 더 강하지만, v18과 같은 학습 계열이며 실제 분포에서
같이 실패할 가능성이 있다. 따라서 v18 실제 ADS가 v16보다 상승한 경우에만 마지막
제출 후보로 승격한다. v18이 하락하면 마지막 제출권은 이 계열에 자동으로 쓰지 않고
실제 결과로 component별 실패를 다시 좁힌다.
