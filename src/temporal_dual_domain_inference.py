"""Offline inference and conservative anchor fusion for temporal MIL heads."""
...
try:
    from .temporal_dual_domain_head import TemporalDualDomainHead
...
def _load_statistics(path: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    return values["ids"].astype(str), values["statistics"], values["view_mask"]
...
def apply_temporal_dual_domain_fusion(
    ...
    voice_weight: float = 0.05,
    music_weight: float = 0.05,
    music_checkpoint_paths: list[Path] | None = None,
) -> None:
    ...
    def predict_checkpoint(path: Path) -> np.ndarray:
        ...
        for offset in range(0, len(eat_ids), batch_size):
            ...
            eat_mask_batch = torch.from_numpy(eat_mask[offset:offset + batch_size]).to(target_device)
            spear_mask_batch = torch.from_numpy(spear_mask[offset:offset + batch_size]).to(target_device)
            with torch.autocast(
                device_type=target_device.type, dtype=torch.bfloat16,
                enabled=target_device.type == "cuda",
            ):
                task, joint, _, _ = model(
                    eat_batch, spear_batch, eat_mask_batch, spear_mask_batch
                )
            batches.append(model.probabilities(task.float(), joint.float()).cpu())
        result = torch.cat(batches).numpy()
    ...
    member_predictions = [predict_checkpoint(path) for path in checkpoint_paths]
    expert = np.mean(member_predictions, axis=0)
    if music_checkpoint_paths:
        music_members = [predict_checkpoint(path) for path in music_checkpoint_paths]
        expert[:, 1] = np.mean(music_members, axis=0)[:, 1]
    expert_by_id = {sample_id: expert[index] for index, sample_id in enumerate(eat_ids)}
